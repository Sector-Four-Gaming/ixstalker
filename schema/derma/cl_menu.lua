...
function PANEL:Init()
	...
	self.buttons:SetSize(self:GetWide() * 0.20, self:GetTall() - self:GetPadding() * 2)
	...
	characters:SetText("▲")
	characters:Dock(TOP)
	characters:SetSize(characters:GetWide(), characters:GetTall() * 0.5)
	...
	close:SetFont("stalkertitlefont")
	close:SetText("ᴥ")
	close:Dock(TOP)
	close:SetSize(close:GetWide(), close:GetTall() * 0.5)
